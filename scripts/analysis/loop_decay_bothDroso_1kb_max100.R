...
bothDroso_loops <- readRDS("data/processed/hic/hg38/diffLoops/bothDroso/diffLoops_bothDroso_10kb.rds") |>
  interactions() |>
...
mcols(bothDroso_loops) <- cbind(as.matrix(assay(bothDroso_loops)), mcols(bothDroso_loops)[-c(1:6)])

## create an mcol for loop size
mcols(bothDroso_loops)$loop_size <- pairdist(bothDroso_loops)

## create an mcol for loop type
mcols(bothDroso_loops)$loop_type <- case_when(
  mcols(bothDroso_loops)$padj < 0.05 & mcols(bothDroso_loops)$log2FoldChange > 1 ~ "truegained",
  mcols(bothDroso_loops)$padj < 0.1 & mcols(bothDroso_loops)$log2FoldChange > 0 ~ "gained",
  mcols(bothDroso_loops)$padj < 0.1 & mcols(bothDroso_loops)$log2FoldChange < 0 ~ "lost",
  mcols(bothDroso_loops)$padj > 0.1 ~ "static",
  is.character("NA") ~ "other")

## create aggregated sorb counts mcol
mcols(bothDroso_loops)$sorb_contacts <- mcols(bothDroso_loops)$YAPP_HEK_sorbitol_4_2_inter_30.hic +
  mcols(bothDroso_loops)$YAPP_HEK_sorbitol_5_2_inter_30.hic + mcols(bothDroso_loops)$YAPP_HEK_sorbitol_6_2_inter_30.hic

## create aggregated cont counts mcol
mcols(bothDroso_loops)$cont_contacts <- mcols(bothDroso_loops)$YAPP_HEK_control_1_2_inter_30.hic +
  mcols(bothDroso_loops)$YAPP_HEK_control_2_2_inter_30.hic + mcols(bothDroso_loops)$YAPP_HEK_control_3_2_inter_30.hic

## create aggregated sorb+cont counts mcol
mcols(bothDroso_loops)$agg_contacts <- mcols(bothDroso_loops)$sorb_contacts + mcols(bothDroso_loops)$cont_contacts

## log transform loop_size to get a normal distribution for matchRanges
mcols(bothDroso_loops)$loop_size <- log(mcols(bothDroso_loops)$loop_size)
hist(mcols(bothDroso_loops)$loop_size) # check for normality

## convert all `0` agg_contact values to NAs and log transform for matchRanges
mcols(bothDroso_loops)$agg_contacts[mcols(bothDroso_loops)$agg_contacts == 0] <- NA
bothDroso_loops <- interactions(bothDroso_loops) |> # no longer InteractionMatrix class
  as.data.frame() |> 
  na.omit() |> 
  as_ginteractions()
  
mcols(bothDroso_loops)$agg_contacts <- log((mcols(bothDroso_loops)$agg_contacts + 1))
hist(mcols(bothDroso_loops)$agg_contacts) # check for normality
...
focal <- bothDroso_loops[!bothDroso_loops$loop_type %in% c("static", "lost","other")] 
pool <- bothDroso_loops[bothDroso_loops$loop_type %in% c("static", "lost","other")] 
...
for(j in seq_along(gainedLoops)){
  l <- 
    ...
    as.matrix()
  for(i in 0:100){
    normalized_gained[j,i+1]<- median(mh_index(buffer = 100, loop = l, inner = i))
  }
}
head(normalized_gained)
colnames(normalized_gained) <- as.character(c(0:100))
...
for(i in seq_along(gainedLoops)){
  for(j in 1:101){
    a <- normalized_gained[i,1]-normalized_gained[i,101]
    b <- normalized_gained[i,j]-normalized_gained[i,101]
    zero_gained[i,j] <- round((b/a),2)
  }
  
}
head(zero_gained)
colnames(zero_gained) <- as.character(c(0:100))


Group_gained <- factor(0:100)
normalized_gained <- na.omit(zero_gained)
saveRDS(normalized_gained, "data/processed/hic/hg38/loop_decay/bothDroso/gainedLoops_bothDroso_1kb_max100_mh_index.rds")
...
for(j in seq_along(nullSet)){
  l <- 
    ...
    as.matrix()
  for(i in 0:100){
    normalized_nullSet[j,i+1]<- median(mh_index(buffer = 100, loop = l, inner = i))
  }
}

head(normalized_nullSet)
colnames(normalized_nullSet) <- as.character(c(0:100))

#Convert data by setting 100 kb as 0 (subtract 100kb value from all values) and center pixel as 1 (divide all values to center pixel)
zero_nullSet=data.frame()
for(i in seq_along(nullSet)){
  for(j in 1:101){
    a <- normalized_nullSet[i,1]-normalized_nullSet[i,101]
    b <- normalized_nullSet[i,j]-normalized_nullSet[i,101]
    zero_nullSet[i,j] <- round((b/a),2)
  }
}
head(zero_nullSet)
colnames(zero_nullSet) <- as.character(c(0:100))


Group_nullSet<- factor(0:100)
normalized_nullSet <- na.omit(zero_nullSet)
saveRDS(normalized_nullSet, "data/processed/hic/hg38/loop_decay/bothDroso/nullSet_bothDroso_1kb_max100_mh_index.rds")
...
for(j in seq_along(dmLoops)){
  ...
  for(i in 0:100){
    normalized_dm[j,i+1]<- median(mh_index(buffer = 100, loop = l, inner = i), na.rm = T)
  }
}

colnames(normalized_dm) <- as.character(c(0:100))
head(normalized_dm)
#Convert data by setting 100 kb as 0 (subtract 100kb value from all values) and center pixel as 1 (divide all values to center pixel)
zero_dm=data.frame()
for(i in seq_along(dmLoops)){
  for(j in 1:101){
    a <- normalized_dm[i,1]-normalized_dm[i,21]
    b <- normalized_dm[i,j]-normalized_dm[i,21]
    zero_dm[i,j] <- round((b/a),2)
  }
  
}

colnames(zero_dm) <- as.character(c(0:100))
head(zero_dm)

Group_dm <- factor(0:100)
normalized_dm <- na.omit(zero_dm)
saveRDS(normalized_dm, "data/processed/hic/dm6/loop_decay/dmLoops_1kb_max100_mh_index.rds")

# load processed data and convert to dataframes ---------------------------

normalized_gained <- readRDS("data/processed/hic/hg38/loop_decay/bothDroso/gainedLoops_1kb_max100_mh_index.rds")
...
normalized_nullSet <- readRDS("data/processed/hic/hg38/loop_decay/bothDroso/nullSet_1kb_max100_mh_index.rds")
## convert to dataframe
Group_nullSet <- factor(0:100)
...
ggsave("plots/loop_decay_bothDroso_1kb_max100.pdf", loop_decay_plot, device = "pdf")
